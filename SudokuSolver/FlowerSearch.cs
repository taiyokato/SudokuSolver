...
namespace SudokuSolver
{
    /// <summary>
    /// 思考実験段階ソースコード。
    /// 中央の空きマスから上下左右にポインタ移動して可能性を埋めていく。
    /// ポインタはQueue[LogItem2]にキューされてるオブジェクトを順番に検索していく。
    /// Dequeue()されたものはStack[LogItem2]のスタックにバックトラック用のスタックに入れる。
    /// 可能性数値がない場合、バックトラック用スタックから逆戻りする
    /// </summary>
    public partial class FlowerSearch
    {
        #region def
        /// <summary> 
        /// Main grid
        ...
        public Queue<LogItem2> ToSearchQueue;
        public Stack<LogItem2> SearchedStack;
        #endregion

        public FlowerSearch(string[,] g, int sbw, int[] fullints, int[,][] tempgrid, int unfilled)
        {
            Grid = g;
        ...
        {
            //GetFilledCount();
            ToSearchQueue = new Queue<LogItem2>();
            SearchedStack = new Stack<LogItem2>();
        ...
        private Point CenterEmpty()
        {
            //if odd grid
            if (FullGridWidth % 3 == 0)
            {
                int c = SingleBlockWidth + 1;
                return new Point() { x = c, y = c };
            }
            else
            {
                int c = (int)(Math.Round((double)(FullGridWidth / 2)));
                return new Point() { x = c, y = c };
            }
        ...
        public void Execute2()
        {
            ...
            STEP1:
                ...
                else
                {
                    ...
                    Next = GetNextEmpty(Next.x, Next.y );
                    goto STEP0;
                }
            }
            catch (Exception ex)
        ...
        #endregion
        #region Get
        private Point GetNextEmpty(int startx, int starty)
        {
            return new Point();
        }
        private Point[] GetNeighbors(LogItem2 li2)
        {
            Queue<Point> Neighbors = new Queue<Point>();
            if ((li2.Point.x==-1)&&(li2.Point.y==-1))
            {
                Neighbors.Enqueue(new Point(li2.Point.x,li2.Point.y-1));
                Neighbors.Enqueue(new Point(li2.Point.x, li2.Point.y + 1));
                Neighbors.Enqueue(new Point(li2.Point.x - 1, li2.Point.y));
                Neighbors.Enqueue(new Point(li2.Point.x + 1, li2.Point.y));
            }
            return Neighbors.ToArray();
        }

        private Direction GetDirection(LogItem2 li)
        {
    ...
    public class LogItem2
    {
        public Point Point;
        public int Value;
        public LogItem2 Parent;
        public LogItem2(Point p, int v, LogItem2 par)
        {
            Point = p;
            Value = v;
            Parent = par;
