...
namespace SudokuSolver
{
    public class Random
    {
        ...
        public Random(string[,] g, int sbw)
        {
            Grid = g;
        ...
        {
            Point[] empties = GetAllEmpty();
            ImpossibleGrid = TempGrid;//get
            ...
            {
            STEP1:
                Next = empties[rnd.Next(0, empties.Length - 1)];
                if (Next.x == -1)
                {
        ...
        private Point[] GetAllEmpty()
        {
            ...
            for (int x = 0; x < FullGridWidth; x++)
            {
                for (int y = 0; y < FullGridWidth; y++)
                {
                    if (Grid[x, y].Equals("x")) tracker.Add(new Point() { x = x, y = y });
                }
            }
